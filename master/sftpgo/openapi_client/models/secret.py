...
class Secret(BaseModel):
    ...
    mode: Optional[StrictInt] = Field(
        None, description="1 means encrypted using a master key"
    )
    __properties = ["status", "payload", "key", "additional_data", "mode"]
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in (
            ...
            raise ValueError(
                "must be one of enum values ('Plain', 'AES-256-GCM', 'Secretbox', 'GCP', 'AWS', 'VaultTransit', 'AzureKeyVault', 'Redacted')"
            )
        return value
