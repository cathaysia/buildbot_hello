...
class GCSConfig(BaseModel):
    ...
    automatic_credentials: Optional[StrictInt] = Field(
        None,
        description="Automatic credentials:   * `0` - disabled, explicit credentials, using a JSON credentials file, must be provided. This is the default value if the field is null   * `1` - enabled, we try to use the Application Default Credentials (ADC) strategy to find your application's credentials ",
    )
    ...
    @validator("automatic_credentials")
    def automatic_credentials_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
    ...
    @classmethod
    def from_json(cls, json_str: str) -> GCSConfig:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> GCSConfig:
        """Create an instance of GCSConfig from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return GCSConfig.parse_obj(obj)

        _obj = GCSConfig.parse_obj(
            {
                "bucket": obj.get("bucket"),
                "credentials": Secret.from_dict(obj.get("credentials"))
                if obj.get("credentials") is not None
                else None,
                "automatic_credentials": obj.get("automatic_credentials"),
